...
function freqQuery(queries) {
  ...
  for (let i = 0; i < queries.length; i++) {
    ...
    // if queries[0] === 1 && queries[0] in cache
    if (queries[i][a] === 1 && cache[queries[i][b]]) {
      // increase frequency
      cache[queries[i][b]]++;
    } else if (queries[i][a] === 1 && !cache[queries[i][b]]) {
      // else if queries[0] && queries[0] not in cache
      // add to cache with frequency amount
      cache[queries[i][b]] = 1;
    } else if (queries[i][a] === 2 && cache[queries[i][b]]) {
      // else if queries[2] === 2 && queries[0] in cache
      // reduce frequency amount by 1
      if (cache[queries[i][b]] === 0) {
        continue;
      } else {
        cache[queries[i][b]]--;
      }
    } else if (queries[i][a] === 2 && !cache[queries[i][b]]) {
      // else if queries[2] === 2 && queries[0] not in cache
      // continue
      continue;
    } else if (queries[i][a] === 3) {
      retArr.push(checkB(cache, queries[i][b]));
    }
  }
  return retArr;
}
